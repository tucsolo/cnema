		  ...
		  unsigned int *col)
{
	...
	*port = 0;
	if ((argc == 2) || (argc == 4) || (argc == 6) || (argc > 7))
	{
			prinf("\tUsage:");
			prinf
			    ("\t\t./server [-r <row size> -c <column size> | -s <side>] -p <port> [-h]\n\n");
			exit(EXIT_SUCCESS);
		}
	while (n < argc) {
		if (!strncmp(argv[n], "-r", 2) || !strncmp(argv[n], "-R", 2)) {
			...
			*port = strtol(argv[++n], NULL, 10);
			...
			if (*port >= 65536)
				eonerror
				    ("Port out of range. Valid range: 1 - 65535 (1-1024 only as root)");
			if (*port == 0)
				*port = 4321;
...
struct cinema *initcinema(unsigned int cols, unsigned int rows)
{
	...
	struct cinema *res;
	if ((res = malloc(sizeof(struct cinema))) == NULL)
	 ...
	 */
	if (seats == NULL)
		eonerror("malloc cinema seats array");
	if (index == NULL)
		eonerror("malloc cinema index array");
	res->rows = rows;
	res->cols = cols;
	res->seat = seats;
	res->indexes = index;
...
void printcinema(struct cinema *room, unsigned int res_id, int fd)
{
	...
	if (res_id != 0)
		snprintf(tempmsg, 511,
			 "\n\tCinema status for reservation id " cver
	...
	for (i = 0; i < room->rows; i++) {
		...
		for (j = 0; j < room->cols; j++) {
			...
			if (seat == 0)
				snprintf(tempmsg, 512, cbia "_");
			else if (seat == res_id)
				snprintf(tempmsg, 512, cver "*");
			else
				snprintf(tempmsg, 512, cros "#");
			send_msg(fd, tempmsg);
		}
	}
...
int reserve(char *message, struct cinema *room, int fd)
{
	...
	while (1 > 0) {
		...
		if ((row <= MAX_ROW) && (col <= MAX_COL)) {
			if (room->seat[row * room->cols + col] == 0)
				room->seat[row * room->cols + col] = index;
			else if (room->seat[row * room->cols + col] != index)
				rem++;
		}
	}
...
void cancels(char *message, struct cinema *room, int fd)
{
	...
	while (1 > 0) {
		...
		row = strtol(lastchar + 1, &thischar, 10);
		col = strtol(thischar + 1, &lastchar, 10);
		if (room->seat[row * room->cols + col] == index)
			room->seat[row * room->cols + col] = 0;
	}
	//      reusing row variable as index
	row = 0;
	while (1 > 0) {
		if (room->seat[row] == index)
			break;
...
void cancela(char *message, struct cinema *room, int fd)
{
	...
	index = strtol(message + 1, &thischar, 10);
	room->indexes[index] = 0;
	for (i = 0; i < room->cols * room->rows; i++)
		if (room->seat[i] == index)
			room->seat[i] = 0;
...
void *serveclient(void *arg)
{
	...
	while (recv_line(fd, tempbuf2, sizeof(tempbuf)) > 0) {
		prsoc(tempbuf2, fd);
		if (tempbuf2[0] == 'P') send_msg(fd, "Pong!");
		if (tempbuf2[0] == 'x')
			break;
		if (tempbuf2[0] == 'h') {
			...
			if (tempbuf2[0] == 'r')
				reserve(tempbuf2, room, fd);
			if (tempbuf2[0] == 'f')
				fill(tempbuf2, room, fd, 0, 0);
			if (tempbuf2[0] == 'c')
				cancels(tempbuf2, room, fd);
			if (tempbuf2[0] == 'd')
				cancela(tempbuf2, room, fd);
			if (tempbuf2[0] == 'u')
				unfill(tempbuf2, room, fd);
			if (tempbuf2[0] == 'z')
				checkzeros(room);
			if (tempbuf2[0] == 'i') {
...
void thread_spawn(struct th_data *arg, struct cinema *room)
{
	...
	if ((pthread_create(&(arg->th_tid), &attr, &serveclient, arg)) != 0)
...
int main(int argc, char *argv[])
{
	...
	if (port == 0) {
		...
		prinf
		    ("To change it, launch this server with ./server -p <port>\n");
	}
	...
	if ((chtemp = malloc(sizeof(char) * 8)) == NULL)
		eonerror("malloc");

	if (ccol == 0)
		while (1 > 0) {
			printf(cbia "\tInsert column size\t[1 - %d]: ",
			       MAX_COL);
	...
	if (listsock < 0)
		eonerror("listening socket creation");
	...
	if (bind(listsock, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0) {
		...
		while (bind
		       (listsock, (struct sockaddr *)&servaddr,
			sizeof(servaddr)) < 0) {
			port++;
			if (port == 65536)
				eonerror("bind error");
	...
	if (listen(listsock, backlog) < 0)
		eonerror("listen error");

	int optval = 1;
	if (setsockopt
	    (listsock, SOL_SOCKET, SO_REUSEPORT, &optval, sizeof(optval)) < 0)
		eonerror("setsockopt SO_REUSEPORT");
	if (setsockopt
	    (listsock, SOL_SOCKET, SO_KEEPALIVE, &optval, sizeof(optval)) < 0)
		eonerror("setsockopt SO_KEEPALIVE");

	prinf("Server started at port ");
	printf(ccia "%d\n", port);
	while (1 > 0) {
		if (thread_count >= MAX_THR)
			pthread_cond_wait(&maxthread_cond, &maxthread_mutex);
		prinf("A new thread is awaiting for connections");

		sockstr = sizeof(struct sockaddr);
		socket_fd =
		    accept(listsock, (struct sockaddr *)&clientaddr, &sockstr);
		if (setsockopt
		    (socket_fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,
		     sizeof(timeout)) < 0)
			eonerror("setsockopt SO_RCVTIMEO");
		if (setsockopt
		    (socket_fd, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout,
		     sizeof(timeout)) < 0)
			eonerror("setsockopt SO_SNDTIMEO");
		prinf("Connection incoming!");
		new_thdata = malloc(sizeof(struct th_data));
		if (new_thdata == NULL)
			eonerror("malloc thread data struct");
